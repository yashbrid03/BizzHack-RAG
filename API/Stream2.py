...
# Initialize LLM without the stdout callback
llm = ChatGroq(
...
# def generate_stream(query: str, namespace: str) -> Generator[str, None, None]:
...
#         # Create a new LLM instance with the streaming callback for this request
...
#         # Since we're using the callback, we need to get the accumulated tokens
#         # But for real streaming, we need to modify this approach
#         for token in streaming_callback.get_tokens():
...
@app.route('/query_non_streaming', methods=['POST'])
def query_rag_non_streaming():
